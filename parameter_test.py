...
model.to(device)
for layer_name in layer_randomization_order:
    model.load_state_dict(torch.load('./models/' + layer_name + '.pth'))
    model = model.eval()
    output = model(input)
    ...
    saliency_methods = ['Gradient', 'Smooth Grad', 'Input X Gradient',
                        'Guided Backpropagation', 'Integrated Gradients', 'DeepLift', 'GradientShap'] #'Guided GradCAM'
    for i in range(len(saliency_methods)):
        ...
        #     guided_gradcam = GuidedGradCam(model, gc_layer)
        ...
        _[0].savefig('./cs/'+ layer_name +'_'+ saliency_method + '.png')
